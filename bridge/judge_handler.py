...
def _ensure_connection():
    try:
        database.connection().cursor().execute('SELECT 1')
    except Exception:
...
class JudgeHandler(ZlibPacketHandler):
    ...
    def on_disconnect(self):
        ...
        if self._working:
            Submission.update(status='IE', result='IE', error='').where(Submission.id == self._working).execute()
            json_log.error(self._make_json_log(sub=self._working, action='close', info='IE due to shutdown on grading'))

    def _authenticate(self, id, key):
        try:
            judge = Judge.get(Judge.name == id)
    ...
    def _update_ping(self):
        try:
            Judge.update(ping=self.latency, load=self.load).where(Judge.name == self.name).execute()
    ...
    def get_related_submission_data(self, submission):
        ...
        try:
            ...
            #                                'user__id', 'contest__participation__virtual',
    ...
    def on_submission_wrong_acknowledge(self, packet, expected, got):
        ...
        Submission.update(status='IE', result='IE', error=None).where(Submission.id == expected).execute()
        Submission.update(status='IE', result='IE', error=None).where(Submission.id == got, Submission.status == 'QU')\
            .execute()
    ...
    def on_supported_problems(self, packet):
        ...
        self.judge.problems.set(Problem.filter(Problem.code.in_(list(self.problems.keys()))))
    ...
    def on_grading_end(self, packet):
        ...
        try:
            submission = Submission.get(Submission.id == packet['submission-id'])
        except Submission.DoesNotExist:
            logger.warning('Unknown submission: %s', packet['submission-id'])
    ...
    def on_compile_error(self, packet):
        ...
        if Submission.update(status='CE', result='CE', error=packet['log'])\
                .where(Submission.id == packet['submission-id']).execute():
    ...
    def on_compile_message(self, packet):
        ...
        if Submission.update(error=packet['log']).where(Submission.id == packet['submission-id']).execute():
    ...
    def on_internal_error(self, packet):
        ...
        id = packet['submission-id']
        if Submission.update(status='IE', result='IE', error=packet['message']).where(Submission.id == id).execute():
    ...
    def on_submission_terminated(self, packet):
        ...
        if Submission.update(status='AB', result='AB', points=0).where(Submission.id == packet['submission-id'])\
                .execute():
    ...
    def on_test_case(self, packet, max_feedback=SubmissionTestCase._meta.fields['feedback'].max_length):
        ...
        if not Submission.update(current_testcase=max_position + 1).where(Submission.id == id).execute():
            logger.warning('Unknown submission: %s', id)
        ...
        SubmissionTestCase.bulk_create(bulk_test_case_updates)
